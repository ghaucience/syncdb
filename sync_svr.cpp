...
static json_t *db_sync_base64_code(void *data, int len);
...
int db_sync(const char *ip, int port, const char *dbpath) {
	...
	for (i = 0; i <= sizeof(tss)/sizeof(tss[0]);) {
		...
		json_t * jsync = db_sync_base64_code(data, ts->size*count)
		if (jsync == NULL) {
			free(data);
			return -3;
		}
...
static int db_sync_get_unsync(stTableSts_t *ts, int count, void **data) {
	...
	uint32_t cnt = count;
	int size = sizeof(dbi_sizes[i]*cnt);
...
static int db_sync_clr_unsync(stTableSts_t *ts, int count, void *data) {
	...
	for (i = 0; i < cnt; i++) {
		...
		for (j = 0; j < ts->condcnt; j++) {
			...
			len += sprintf(where + len, "%s%s = '%s'", ax, pi + cond->offset);
		}
